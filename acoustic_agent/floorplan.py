...
# Keep the upstream archive filename while exposing the converted feature as Floorplan.
DEFAULT_FLOORPLAN_PATH = Path(__file__).resolve().parents[2] / "ResPlan.pkl"
...
class _FloorplanUnpickler(pickle.Unpickler):
    def find_class(self, module: str, name: str) -> Any:
        if (module, name) not in _ALLOWED_PICKLE_GLOBALS:
            raise pickle.UnpicklingError(f"unsupported Floorplan pickle global: {module}.{name}")
        return super().find_class(module, name)


class FloorplanDataset:
    def __init__(self, path: str | Path = DEFAULT_FLOORPLAN_PATH) -> None:
        self.path = Path(path).expanduser().resolve()
        if not self.path.is_file():
            raise FileNotFoundError(f"Floorplan dataset not found: {self.path}")
        with self.path.open("rb") as handle:
            records = _FloorplanUnpickler(handle).load()
        if not isinstance(records, list):
            raise ValueError("Floorplan dataset must contain a list of floor plans")
        ...
        if not self.eligible_indices:
            raise ValueError("Floorplan dataset contains no eligible single-floor scenes")
    ...
    def scene(
        ...
        *,
        ...
        if index < 0 or index >= len(self.records):
            raise IndexError(f"Floorplan index must be between 0 and {max(0, len(self.records) - 1)}")
        profile = self.profiles[index]
        if not profile["eligible"]:
            reasons = ", ".join(profile["filter_reasons"])
            raise ValueError(f"Floorplan[{index}] is filtered: {reasons}")
...
def scene_from_record(
    ...
    *,
    ...
    if not isinstance(graph, nx.Graph):
        raise ValueError(f"Floorplan[{index}] has no valid room graph")
    ...
    if not rooms:
        raise ValueError(f"Floorplan[{index}] contains no valid supported rooms")
...
def _multi_room_scene_from_record(
    ...
    *,
    ...
    if inner is None:
        raise ValueError(f"Floorplan[{index}] has no valid global indoor boundary")
    ...
    if global_polygon is None:
        raise ValueError(f"Floorplan[{index}] has no valid metric indoor boundary")
    ...
    room_metadata = {
        "shape": "floorplan",
        "geometry_model": "floorplan_multi_room_extrusion",
        "opening_model": "vertical_portal_apertures_v1",
        "connectivity_model": "verified_floorplan_portal_graph_v1",
