...
DEFAULT_FLOORPLAN_RESOURCE = Path(__file__).resolve().parent / "resources" / "floorplan" / "floorplan_v1.sqlite3"
_EXTERIOR_CONNECTIONS = {"outdoor_entry", "outdoor_facade", "outdoor_balcony"}


class FloorplanResource:
    """Random-access loader for precompiled, losslessly compressed Floorplan scenes."""

    def __init__(self, path: str | Path = DEFAULT_FLOORPLAN_RESOURCE) -> None:
        self.path = Path(path).expanduser().resolve()
        if not self.path.is_file():
            raise FileNotFoundError(
                f"compiled Floorplan resource not found: {self.path}; "
                "run scripts/build_floorplan_resource.py"
            )
        ...
        if int(self.metadata.get("schema_version", -1)) != RESOURCE_SCHEMA_VERSION:
            raise ValueError(
                f"unsupported Floorplan resource schema {self.metadata.get('schema_version')!r}; "
    ...
    def record(self, index: int) -> dict[str, Any]:
        ...
        if row is None:
            raise IndexError(f"Floorplan resource index must be between 0 and {self.count - 1}")
        return json.loads(zlib.decompress(row[0]))

    def resolve_index(self, index: int, direction: str = "nearest") -> int:
        if self.count < 1:
            raise IndexError("Floorplan resource is empty")
    ...
    def sample_placement(
        ...
        *,
        ...
        if not rooms:
            raise ValueError(f"Floorplan resource index {index} contains no usable rooms")
