...
from acoustic_agent.floorplan import DEFAULT_FLOORPLAN_PATH, FloorplanDataset
from acoustic_agent.floorplan_resource import DEFAULT_FLOORPLAN_RESOURCE, RESOURCE_SCHEMA_VERSION
...
def build_resource(source: Path, destination: Path) -> None:
    ...
    dataset = FloorplanDataset(source)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile the filtered Floorplan pickle into a random-access resource.")
    parser.add_argument("--source", type=Path, default=DEFAULT_FLOORPLAN_PATH)
    parser.add_argument("--output", type=Path, default=DEFAULT_FLOORPLAN_RESOURCE)
